...
const SearchType = () => {
  ...
  const handleTypeSelection = (type) => {
    ...
    } else if (!secondType) { //Si el estado secondType está vacío, significa que ya se ha seleccionado un tipo y se está seleccionando un segundo tipo, entonces se actualiza el estado secondType con el tipo seleccionado.
      setSecondType(type);
    } else {
  ...
  return (
    <div>
        <form onSubmit={submitHandler} className={style.main}>
            {types.map(type=>{   
                return <button className={style.button} onClick={()=>handleTypeSelection(type.name)}><img className={style.icon} src={typeIcons[type.name]}/></button> //Dentro de la función handleTypeSelection, se están actualizando los estados firstType y secondType con los valores correspondientes, lo cual permite realizar la búsqueda con los dos tipos seleccionados.
            })}
        </form>
    </div>
  );
